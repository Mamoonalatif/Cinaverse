...
export class ReviewsService {
  ...
  async create(userId: number, movieId: string, rating: number, comment: string) {
    const result = await this.repo.save({ movieId, rating, comment, user: { id: userId }, isVisible: true });
    this.apiLogService.createApiLog({ id: userId } as any, '/review', 201).catch(() => { });
    return this.repo.findOne({ where: { id: result.id }, relations: ['user'] });
  }

  getByMovie(movieId: string) {
    return this.repo.find({ where: { movieId, isVisible: true }, relations: ['user'] });
  }
